...
if [ "$1" = "" ] ; then
	...
	EOF
elif [ "$1" = "createdb-patch9" ]; then # db: move recipients field to table dirs_content
	ENGINE=myisam
	mysql -uroot -S $SOCKN $DBNAME <<-EOF
...
elif [ "$1" = "deletenode" ]; then # config: Delete this node in bolixo.org
	...
	cmpsequence "$2" cleartest1 directory createsubdir projects msgs ivldsession public remote-contact remote-interest contact-utf8 notifications \
		...
		doc-photos doc-whiteboard-imbed-nbwait doc-whiteboard-imbed-ivld bolixod-registernode
