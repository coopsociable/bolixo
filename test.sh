...
if [ "$1" = "" ] ; then
	...
	rm -fr $CMPDIR
	mkdir $CMPDIR
	for test in directory createsubdir projects msgs ivldsession public remote-contact remote-interest contact-utf8 notifications remote-sendlarge cp-admin badnames setaccess
	do
		OPT=
		if [ "$test" = "public" ]; then
			OPT=jacques-A
